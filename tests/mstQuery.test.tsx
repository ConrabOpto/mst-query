...
import { merge } from '../src/QueryModelBase';
...
test('deep update of object', () => {
    ...
    const ModelC = types.model({ id: types.identifier, a: types.maybe(types.string) });
    const ModelB = types.model({ a: types.maybe(types.string), b: types.maybe(types.string) });
    const ModelA = types.model({
...
test('replace arrays on sub properties', () => {
    const Model = types.model({
        id: types.identifier,
        prop: types.model({ ids: types.array(types.model({ baha: types.string })) }),
    });
