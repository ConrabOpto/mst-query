...
const useInvoiceQuery = (id: string, initialData: any) => {
  ...
  const invoiceApiStore = rootStore.invoiceService;
  return useQuery(invoiceApiStore.invoiceQuery, {
    request: { id },
    initialData,
    staleTime: 5000
  });
...
export const InvoiceEditor = observer(
  (props: { invoice: InvoiceModelType }) => {
    const { invoice } = props;
